...
def verify_otp(request):
    if request.method == "POST":
        ...
        if str(user_otp)==str(original_otp):
...
def cancel_order(request, order_id, product_id):
    ...
    return render(request,"ecom/cancel_order.html",{"order":order,"product":product})

#for showing login button for admin(by shivam & om)
...
def send_feedback_view(request):
    ...
    return render(request,'ecom/send_feedback.html',{ 'form': form })
...
def sales_report(request):
    ...
    if start_date and end_date:
        generate=True
        if product_name:
            orders=Orders.objects.filter(order_date__range=[start_date,end_date],product__name=product_name)
        else:
            orders=Orders.objects.filter(order_date__range=[start_date,end_date])
    
    return render(request, 'ecom/sales_report.html',{'orders':orders,'products':products,'product_name':product_name,'generate':generate})